...
import { checkAuth, getRepos, getReviews, type Repo, type Review } from "@/lib/api";
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [reposData, reviewsData] = await Promise.all([
        getRepos(),
        getReviews({ repo_id: selectedRepo ?? undefined, per_page: 20 }),
      ]);
    ...
    return (
      <div style={{ background: "#0d1117", minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center", maxWidth: 400 }}>
          ...
          <a href={`${API_URL}/auth/github`} style={{ background: "#238636", color: "#fff", padding: "10px 24px", borderRadius: 6, textDecoration: "none", fontWeight: 600, fontSize: 14 }}>
            Sign in with GitHub
          </a>
  ...
  return (
    <div style={{ background: "#0d1117", minHeight: "100vh", color: "#e6edf3" }}>
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto", padding: "24px", display: "grid", gridTemplateColumns: "220px 1fr", gap: 24 }}>
        <div>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#8b949e", textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 8 }}>Repositories</div>
          <button onClick={() => setSelectedRepo(null)} style={{ width: "100%", textAlign: "left", background: selectedRepo === null ? "rgba(137,87,229,0.1)" : "transparent", border: `1px solid ${selectedRepo === null ? "rgba(137,87,229,0.3)" : "transparent"}`, borderRadius: 6, padding: "7px 10px", color: selectedRepo === null ? "#8957e5" : "#e6edf3", fontSize: 13, cursor: "pointer", marginBottom: 2 }}>
            All repositories
          </button>
          {loading && repos.length === 0 ? (
            <div style={{ color: "#8b949e", fontSize: 12, padding: "8px 0" }}>Loading…</div>
          ) : repos.length === 0 ? (
            <div style={{ color: "#8b949e", fontSize: 12, padding: "8px 0" }}>No repos yet. <a href={INSTALL_URL} style={{ color: "#388bfd" }}>Install PRPilot</a></div>
          ) : (
            repos.map((repo) => (
              <button key={repo.repo_id} onClick={() => setSelectedRepo(repo.repo_id)} style={{ width: "100%", textAlign: "left", background: selectedRepo === repo.repo_id ? "rgba(137,87,229,0.1)" : "transparent", border: `1px solid ${selectedRepo === repo.repo_id ? "rgba(137,87,229,0.3)" : "transparent"}`, borderRadius: 6, padding: "7px 10px", color: selectedRepo === repo.repo_id ? "#8957e5" : "#e6edf3", fontSize: 13, cursor: "pointer", marginBottom: 2, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }} title={repo.repo_full_name}>
        ...
        <div>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 16 }}>
            ...
            <div style={{ display: "flex", gap: 8 }}>
              {selectedRepo && (
                <Link href={`/dashboard/repos/${selectedRepo}`} style={{ color: "#388bfd", fontSize: 13, textDecoration: "none", padding: "5px 12px", border: "1px solid #30363d", borderRadius: 6 }}>Configure →</Link>
              )}
              <button onClick={load} style={{ background: "transparent", border: "1px solid #30363d", color: "#8b949e", borderRadius: 6, padding: "5px 12px", fontSize: 13, cursor: "pointer" }}>↻ Refresh</button>
            </div>
          </div>

          {error && <div style={{ background: "rgba(218,54,51,0.1)", border: "1px solid rgba(218,54,51,0.3)", borderRadius: 6, padding: "10px 14px", color: "#da3633", fontSize: 13, marginBottom: 16 }}>{error}</div>}
          ...
          {loading ? (
            ...
            <div style={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 8, padding: "48px 24px", textAlign: "center" }}>
              <div style={{ maxWidth: 480, margin: "0 auto" }}>
                <div style={{ fontWeight: 700, fontSize: 16, color: "#e6edf3", marginBottom: 24 }}>No reviews yet — here is how to get started</div>
                {[
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {reviews.map((review) => (
                <Link key={review.id} href={`/dashboard/reviews/${review.id}`} style={{ textDecoration: "none" }}>
                  <div style={{ background: "#161b22", border: "1px solid #30363d", borderRadius: 8, padding: "14px 16px", display: "grid", gridTemplateColumns: "1fr auto", gap: 12, cursor: "pointer", transition: "border-color 0.15s" }}
                    ...
                    <div>
                      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
                        ...
                        {review.language && <span style={{ fontSize: 11, color: "#8957e5", background: "rgba(137,87,229,0.1)", border: "1px solid rgba(137,87,229,0.2)", borderRadius: 10, padding: "0 7px" }}>{review.language}</span>}
                      </div>
                    ...
                    <div style={{ display: "flex", alignItems: "center" }}><VerdictBadge verdict={review.verdict} /></div>
                  </div>
                </Link>
              ))}
            </div>
